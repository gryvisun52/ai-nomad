const Bridge = {
    ...
    showReward(cb) { if(confirm("Смотреть рекламу для ТУРБО?")) cb(true); },
    save(d) { localStorage.setItem('ai_nomad_v16_final', JSON.stringify(d)); }
};
...
function addXP(a) {
    ...
    while (state.xp >= state.nextXp) {
        ...
        playSnd('snd-level'); showToast(`LVL UP!`);
    }
}
...
window.onload = () => {
    Bridge.init();
    const s = localStorage.getItem('ai_nomad_v16_final');

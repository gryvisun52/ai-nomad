// --- FIREBASE CONFIG (Замени URL на свой) ---
...
const Bridge = {
    ...
    setUserData(n) { state.username = n; localStorage.setItem('ai_nomad_user', n); document.getElementById('auth-overlay').style.display = 'none'; updateUI(); },
    checkAuth() { const s = localStorage.getItem('ai_nomad_user'); if (s) this.setUserData(s); else document.getElementById('auth-overlay').style.display = 'block'; },
    showReward(cb) { if(confirm("Смотреть рекламу для ТУРБО x10?")) cb(true); },
    save(d) { localStorage.setItem('ai_nomad_v_final', JSON.stringify(d)); }
};
...
let bots = [ { name: "CryptoKing", money: 150000, level: 25 }, { name: "NeuralPunk", money: 85000, level: 18 } ];
...
async function registerManual() {
    ...
    if (snap.exists()) { showToast("Ник занят!"); btn.disabled = false; }
    else { await db.ref('users/' + name).set({ registeredAt: new Date().toISOString() }); Bridge.setUserData(name); }
}
...
function addXP(a) {
    state.xp += a;
    while (state.xp >= state.nextXp) { state.xp -= state.nextXp; state.level++; state.nextXp *= 1.6; playSnd('snd-level'); }
}
...
function buyFarm(id, b, i) {
    ...
    if (state.money >= p) { state.money -= p; state.farm[id].count++; state.autoIncome += i; updateUI(); Bridge.save(state); }
}
...
function startResourceProduction(t, bc, s) {
    ...
    state.money -= cost; state.production[t] = true; state.resCosts[t] = Math.floor(state.resCosts[t] * 1.2);
    const bar = document.getElementById(`progress-${t}`);
    bar.style.transition = `width ${s}s linear`; setTimeout(() => bar.style.width = "100%", 50);
    setTimeout(() => { state.res[t]++; state.production[t] = false; bar.style.transition = "none"; bar.style.width = "0%"; updateUI(); Bridge.save(state); }, s * 1000);
    updateUI();
}
...
window.onload = () => { Bridge.init(); const s = localStorage.getItem('ai_nomad_v_final'); if(s) state = {...state, ...JSON.parse(s)}; updateUI(); };
